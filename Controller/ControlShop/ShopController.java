...
public class ShopController implements ShopControllerService {
    ...
    public boolean displayParticularProduct(int phoneId){
        boolean displayed=false;
        HashMap<Integer,Integer>phones=dataSource.getPhones();
        if(!(phones.get(phoneId)==0)){
            phoneController=new PhoneController();
    ...
    public int addProductToShop(){
        ...
        HashMap<Integer,PhoneStorageCapacity>phoneStorageDetails=dataSource.getPhoneStorageCapacityDetails();
        boolean add=shopView.confirmAddProduct();
        if(quantity==0 || (!add)) {
            return 3;
        }
        for (int phoneId : phoneDetails.keySet()) {
            if(phoneDetails.get(phoneId).getModelName().equalsIgnoreCase(phoneController.getModelName()) && phoneStorageDetails.get(phoneId).getStorageCapacity().equalsIgnoreCase(phoneController.getStorageCapacity())) {
        ...
        dataSource.addPhoneToShop(phoneController.getPhoneId(), this.getProductsFromShop().get(phoneController.getPhoneId()));
